...
- **Super-admin platform surface**: `GET/PUT /admin/platform/{features,customer-config,brand}` (`artifacts/api-server/src/routes/platform.ts`) are gated by `requireAuth` **plus** `requireSuperAdmin`, a *stricter* boundary than ordinary admin. Super-admin identity is the set of emails in `SUPER_ADMIN_EMAILS` (CSV env), falling back to the seeded brand-admin email so a fresh deployment is reachable; regular admins are rejected with 403. These endpoints mutate per-deployment branding (company name/colors/logo/contact emails — `platform_brand_config` singleton), feature flags, and commercial plan config. All write bodies are zod-validated: hex colours are format-checked, contact fields are email-validated, and the logo is constrained to an `data:image/*` URI capped ~512 KB (700 KB encoded) to bound body size. The brand override layer never exposes secrets — demo credentials, `salesEmail`, and `privacyEmail` are env-only and not overridable from the UI, and the public `GET /api/brand` returns only presentational brand fields + feature flags, never credentials or super-admin state.

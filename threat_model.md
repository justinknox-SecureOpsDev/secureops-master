...
- **Site-manager per-site scoping (June 2026).** The `site_manager` role (renamed from `lead`) is an employee with scheduling powers confined to assigned sites (`site_managers` join table; `lib/siteManagerAuth.ts`). Every shift / assignment / time-entry surface joins through `site_id` and calls `assertCanManageSite` (admin + dispatcher bypass; fails closed when site membership is unresolved), so a manager cannot read or mutate another site's operational data (IDOR-safe). Cross-site information disclosure is additionally blocked at the data layer: `GET /clients` returns only clients that own a managed site, with billing/contact fields stripped, and a site manager's shift create/edit cannot set `siteRateId` (the rate-card FK is forced null / ignored); moving a shift to another managed site recomputes pay/bill from the destination site's defaults (fail-closed), so no admin-set finance carries across sites even though rates are already stripped from responses. Site managers retain access to their OWN pay/paystubs.

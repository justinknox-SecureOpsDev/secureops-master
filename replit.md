...
- **`schema-drift`** — `check-schema-drift` (`scripts/src/check-schema-drift.ts`). Introspects every pg table exported from `@workspace/db/schema` (via drizzle `getTableConfig`) and asserts each table + column exists in the live DB (`information_schema.columns`). Fails fast — naming each missing table/column — when the code schema is ahead of the DB because a `pnpm --filter @workspace/db run push` was forgotten, so a stale DB can't silently break `test`/`security-headers` later with a cryptic `column ... does not exist`. Extra DB-only columns are ignored (not a release blocker). Needs `DATABASE_URL` (or `OVERRIDE_DATABASE_URL`); exits 1 if the DB is unreachable.

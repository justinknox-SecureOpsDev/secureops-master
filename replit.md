...
- **Analytics** (`/admin-portal/analytics`, admin-only, ungated core): same money rules + CSV/PDF export. Weekly buckets use business-TZ (Central) Mondays — aligned with payroll and invoicing (all three bucket the same Central week), so a Sunday-evening entry charts in the same week it invoices and pays. (The officer-history sub-report labels that same Central week as an ISO `YYYY-Www` string.)
...
- **Branding (super-admin)**: Platform → Branding edits the `platform_brand_config` singleton live (no redeploy); `lib/brandConfig.ts` merges env defaults ← non-null DB overrides, patched in-process. Super-admin = `SUPER_ADMIN_EMAILS` CSV (falls back to seeded admin email). Overridable: company/short/app names, tagline, company license # (`companyLicense`, env `COMPANY_LICENSE` — rendered verbatim only when non-empty across portal header, mobile profile, home footer, email signatures/footers, and all 4 PDFs), 3 brand colors, billing/hr/adminNotify emails, logo (`data:image/*` ≤~512 KB). Env-only: salesEmail, privacyEmail, demo credentials. Public `GET /api/brand` (`no-store`) serves text/colors/logo + feature flags; portal CSS vars and all 4 PDFs read it live. **Portal pre-auth login stays fixed "SecureOps Command"**; **mobile login IS tenant-branded** once an org is connected — the app pulls colors/logo/name/tagline live from that backend's `GET /api/brand`, derives its palette (`constants/colors.ts derivePalette`; WCSG defaults short-circuit to the hand-tuned palette EXACTLY; high-contrast stays fixed), persists per-org brand on-device (hydrated inside the org init barrier so relaunch has no WCSG flash; cleared on org switch), and falls back to the platform lockup pre-connect (connect screen stays neutral).

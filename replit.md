...
- **Invoice generation** (`POST /invoices/generate {siteId, weekStart}`, admin): builds a draft invoice from APPROVED time entries for that site + week (Mon-Sun UTC). Hours = each entry's actual `hoursWorked` (open / 0-hour entries skipped). Rate = `sites.defaultBillRate` (primary, contracted client rate) with `shifts.billRate` as a defensive legacy fallback; if neither resolves > 0 the whole call returns 400 (`Set the site's default bill rate, then retry.`). Includes ad-hoc geo clock-ins via `or(shift.siteId=siteId, time_entries.siteId=siteId)`. Line items grouped per (officer, rate) and read as `"Smith, John -- 16.50h x $42.00"`. Due date = today + `clients.paymentTermsDays`. Site bill rate editable from `/admin-portal/tables/sites` ("Bill Rate ($/hr)").

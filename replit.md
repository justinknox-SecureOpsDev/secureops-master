...
- **License hierarchy**: L2 unarmed → L3 armed (covers L2+L3) → L4/PPO (covers all). `maxLicenseLevel` = MAX(level) of unexpired licenses. **Unarmed work (level ≤ 2) is open to EVERY employee** (eligibility floor — see next bullet); armed (3) and L4/PPO (4) still require the matching unexpired licence.

...
- `pnpm --filter @workspace/scripts run a11y` — accessibility regression scan (axe-core + Playwright/Chromium) over the Admin Portal's key surfaces (public Apply / Onboard / Amend forms, a DataGrid table page, the Import wizard dialog). Fails on any critical/serious WCAG 2.1 A/AA violation; mints its own short-lived onboarding/amendment tokens. **Self-bootstrapping:** it brings up whatever it needs — spawns the `api-server` + `admin-portal` dev servers itself (on their artifact-declared ports 8080 / 25580 behind the shared proxy) if they aren't already running, waits for them to be reachable, and ensures the documented admin account exists (auto-seeds the default `admin@secureops.com` when no custom creds are given) — then tears down anything it started on exit. So it passes from a cold environment with no manual workflow start. If a prerequisite genuinely can't be brought up it fails with a clear `prerequisite not ready:` message that is distinct from a real accessibility violation (no false negatives in CI). Login retries briefly on transient proxy 5xx so a service flapping mid-restart isn't mistaken for bad credentials. Override with `A11Y_BASE_URL`, `A11Y_ADMIN_EMAIL`, `A11Y_ADMIN_PASSWORD` (supplying custom admin creds skips auto-seed — the account is used as-is). Chromium is resolved from the Nix system binary (`which chromium`) or `PLAYWRIGHT_CHROMIUM`. **Registered as the `a11y` validation step (CI gate), so it runs automatically on validation / pre-deploy — now self-contained like the other gates.**
- `pnpm --filter @workspace/scripts run a11y-mobile` — static screen-reader-label lint for the Expo mobile officer screens (`scripts/src/a11y-mobile-labels.ts`). Parses each in-scope officer screen with the TypeScript compiler API and fails if any interactive element (`TouchableOpacity` / `TouchableHighlight` / `TouchableWithoutFeedback` / `Pressable` / `Switch` / `TextInput`) declares neither `accessibilityLabel` nor `accessibilityRole`. Escape hatches: `accessible={false}`, `accessibilityElementsHidden`, `importantForAccessibility`, `aria-hidden`, or a JSX prop spread. No workflow / device / DB required — pure static analysis. Scope is the `OFFICER_SCREEN_GLOBS` allow-list in the script; add screens there as more are made screen-reader friendly. **Registered as the `a11y-mobile` validation step (CI gate), so it runs automatically on validation / pre-deploy — an unlabeled control fails the gate (file + line + element type) and blocks release.**

import { pgTable, text, uuid, timestamp, numeric, boolean, jsonb, index } from "drizzle-orm/pg-core";
...
export const usersTable = pgTable("users", {
  ...
  // Backs the admin grid's default sort (createdAt desc) + id tiebreaker so
  // list loads and the row-position deep-link stay index-ordered at scale.
  createdIdx: index("users_created_idx").on(t.createdAt, t.id),
}));

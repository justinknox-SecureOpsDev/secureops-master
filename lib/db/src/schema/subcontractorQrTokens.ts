...
export const subcontractorQrTokensTable = pgTable("subcontractor_qr_tokens", {
  ...
  /**
   * Optional admin pay rate ($/hr) for this QR code. When set, it represents
   * what WCSG pays the subcontractor company. Null = no per-QR rate set (for
   * admin records only; never exposed to the public clock-in page).
   */
  payRate: numeric("pay_rate", { precision: 10, scale: 2 }),
  /**
   * Optional bill rate ($/hr) applied to client invoices for time entries
   * generated by this QR code. Null = fall back to the site's defaultBillRate.
   * Never exposed to the public clock-in page or public endpoints.
   */

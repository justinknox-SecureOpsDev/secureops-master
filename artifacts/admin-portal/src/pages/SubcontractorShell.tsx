...
import SubcontractorInsurance from "@/pages/SubcontractorInsurance";
import SecurityPage from "@/pages/Security";
import NotFound from "@/pages/not-found";

const NAV = [
  { href: "/subcontractor", label: "Company Profile", Icon: Building2, exact: true },
  { href: "/subcontractor/insurance", label: "Insurance", Icon: ShieldCheck },
];
  ...
  return (
    <div className="flex flex-col h-screen w-full overflow-hidden bg-background">
      ...
      <header className="shrink-0 bg-[#0c0a08] text-white border-b border-[#c9a04a]/30 px-4 py-3 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="hidden md:flex items-center gap-1 ml-6">
            <span className="text-[#f0e4c0]/40 text-xs">Vendor Portal</span>
      ...
      <main className="flex-1 overflow-auto">
        <Switch>
          <Route path="/subcontractor" component={SubcontractorProfile} />
          <Route path="/subcontractor/insurance" component={SubcontractorInsurance} />
          <Route path="/account/security" component={SecurityPage} />
          {/* Root fallback: vendors land on the bare portal base after login.
              Depending on the trailing slash, wouter reports the location as
              "/" or "" — both must route to the profile page. Anything else 404s. */}
          <Route component={SubcontractorRootFallback} />
        </Switch>
      </main>
    </div>
  );
...
function SubcontractorRootFallback() {
  const [location] = useLocation();
  if (location === "" || location === "/") return <Redirect to="/subcontractor" />;
  return <NotFound />;
}

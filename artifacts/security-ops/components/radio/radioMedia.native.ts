...
class NativeRadioMedia implements RadioMedia {
   ...
   * the silent player during the reconnect gap would let iOS suspend a locked
   * phone (~30s) before the retry lands, and a suspended app never retries —
   * the officer would go permanently deaf on exactly the network blip the
   * self-heal exists for. Cleared once the channel is re-listened, dropped, or
   * the recovery listener goes away, so the player can never outlive real
   * demand (APP_REVIEW_NOTES.md §3 promises it stops when the user leaves).
   */
  private recovering = new Set<string>();

  listenChannelIds(): string[] {
    // Deliberately includes channels that are only RECOVERING (room already
    // gone). They still hold keep-alive demand, and the screen releases demand
    // by drop-listing every id here that it no longer wants — so hiding them
    // would strand the silent player whenever the officer mutes or switches
  ...
  setOnListenLost(cb: ((channelId: string) => void) | null): void {
    ...
    // so pending recovery must stop counting as keep-alive demand.
    if (!cb && this.recovering.size > 0) {
  ...
  async ensureListen(channelId: string, token: RadioToken): Promise<void> {
    ...
    try {
      ...
      room.on(this.lkc.RoomEvent.Disconnected, (reason?: DisconnectReason) => {
        ...
        // Recovery is the screen's job, but keep-alive demand must be handed
        // over atomically: without this the delete can strand the silent
        // player as the last demand source (it loops forever with no radio
        // connection), and a plain reconcile here would instead kill it
        // mid-reconnect. Track the channel as recovering, then reconcile.
        if (this.onListenLost) this.recovering.add(channelId);
        this.reconcileKeepAlive();

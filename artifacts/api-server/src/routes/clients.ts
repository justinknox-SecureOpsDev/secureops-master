...
// Financial client fields a site manager must never see (billing terms /
// address). Everything else (name + contacts) is needed to pick a client when
// scheduling.
...
// Site managers need the client list to scope the site picker when creating
// shifts, but with billing/payment-terms stripped. Admins see the full row.
